...
public class ReciteWordControl extends ReciteWord implements JSONBasedControl {
    ...
    public void analysisReciteResult() throws SQLException{
        ...
        JSONArray wordDataList = new JSONArraytron( szWordData );
        ...
        for( int i = 0 ;i<wordDataList.length();i++) {
            ...
            this.mysql().updateWithArray(this.alchemist().user().glossary().tabWordsNS(),dataMap,condition);
        }
        ...
        this.mPageData.put( "WordPropertyList", new JSONMaptron(mproperty) );
        this.mPageData.put( "WordTestData", new JSONMaptron(wordTestData) );
    }
    ...
    private JSONArray wrongTimeRank( Integer[] WrongTimeWord,JSONArray wordDataList ){
        JSONArray wordWrongTimeRank = new JSONArraytron();
