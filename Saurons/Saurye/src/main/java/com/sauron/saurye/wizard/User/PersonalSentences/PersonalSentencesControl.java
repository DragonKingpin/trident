...
//        );
    ...
    protected boolean saveSentenceWord( Map<String, String > map, String szWord ) throws SQLException {
        if( this.alchemist().mutual().word().hasSuchWord( szWord ) ){
            ...
            return this.mysql().insertWithArray( this.alchemist().user().pamphlet().sentence().tabEnSentWordsNS(), map ) > 0;
        }
        return true;
    }
    ...
    public void makeSentence() throws IOException, SQLException {
        ...
        if( !$_SPOST.isEmpty() && !StringUtils.isEmpty( $_SPOST.optString( "index_of" ) ) ){
            ...
            String szMId       = (new Randomium()).nextString(20 );
            ...
            boolean bRes = this.mysql().insertWithArray( this.alchemist().user().pamphlet().sentence().tabEnSentencesNS(), data.getMap() ) > 0;
    ...
    public void sentenceModify() throws IOException,SQLException {
        ...
        if( !$_SPOST.isEmpty() && !StringUtils.isEmpty( $_SPOST.optString( "index_of" ) ) ){
            ...
            boolean bRes = this.mysql().updateWithArray( this.alchemist().user().pamphlet().sentence().tabEnSentencesNS(), data.getMap(), conditionS ) > 0;
    ...
    public boolean deleteOneSentence( String szID ) throws IOException, SQLException {
        return this.mysql().deleteWithSQL( this.alchemist().user().pamphlet().sentence().tabEnSentencesNS(), String.format(
                " WHERE id='%s' AND username='%s'" , szID, this.mszCurrentUser )
        ) > 0; // Using trigger
    }

    public void sentenceMassDelete() throws IOException, SQLException {
        DMLayerSkill.commonMassDelete( this, "id[]", "deleteOneSentence" );
    }
