...
import com.sauron.saurye.peripheral.Skill.Plug.HanLPFileIoAdapter;
...
public class TextBasicProcessor {
    ...
    public TextBasicProcessor( Saurye host ) {
        this.mHost = host;
    ...
    private void loadStopWord( ){
        if( this.mCnStopWord == null || this.mEnStopWord == null ){
            try{
                this.mCnStopWord = this.getStopWordSet( this.mHost.mysql().tableName( Saurye.TABLE_CN_STOP_WORD ) );
                this.mEnStopWord = this.getStopWordSet( this.mHost.mysql().tableName( Saurye.TABLE_EN_STOP_WORD ) );
            }
