...
//        );
    ...
    public boolean abortOneCollection( String szClassId ) throws SQLException {
        ...
        return this.mysql().deleteWithSQL( this.alchemist().user().pamphlet().tabCollectionNS(), szCondition ) > 0;
    }
    ...
    public void appendNewWord() throws IOException,SQLException {
        ...
        if( this.glossaryIsReciting(szClassId) ){
            ...
            for( String word : words ){
                ...
                bRes &= this.mysql().insertWithArray( this.alchemist().user().glossary().tabWordsNS(), wordData.getMap() ) > 0;
            }
...
//        }
    ...
    public boolean deleteOneWord( String szID ) throws IOException, SQLException {
        return this.mysql().deleteWithSQL( this.alchemist().user().glossary().tabWordsNS(), String.format(
    ...
    public void editWordProfile() throws SQLException,IOException {
        ...
        this.checkResult(
                this.mysql().updateWithArray( this.alchemist().user().glossary().tabWordsNS(), data.getMap(),condition ) > 0
        );
    }
        ...
        if( jGlossaryWordList != null ){
            ...
            for( int i = 0; i < jGlossaryWordList.length(); i++ ){
                ...
                if( !indexSet.contains( szWord ) ){
                    ...
                    bRes &= this.mysql().updateWithArray( this.alchemist().user().glossary().tabWordsNS(), data, condition ) > 0;
                }
            }
        }
        else {
...
//        return this.mysql().deleteWithSQL( this.alchemist().user().word().tabRecordNS(), String.format(
//                " WHERE id='%s' AND username='%s'" , szID, this.mszCurrentUser )
//        ) > 0;
//    }

    public void historyMassDelete() throws IOException, SQLException {
        DMLayerSkill.commonMassDelete( this, "id[]", "deleteOneHistory" );
    }
    ...
    public boolean forgetOneRecall ( String szWord ) throws IOException, SQLException {
        return this.mysql().deleteWithSQL( this.alchemist().user().word().tabRecallNS(), String.format(
                " WHERE `en_word`='%s' AND username='%s'" , szWord, this.mszCurrentUser )
        ) > 0;
    }

    public void wordsRankMassDelete() throws IOException, SQLException {
        DMLayerSkill.commonMassDelete( this, "en_word[]", "forgetOneRecall" );
    }
