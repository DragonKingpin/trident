...
import com.sauron.saurye.peripheral.Skill.Util.FileHelper;
...
public class PersonalFragmentsControl extends PersonalFragments implements JSONBasedControl {
    ...
    private String appendNewGlossary( boolean bIsCloneMode ) throws IOException,SQLException {
        try {
            ...
            String szClassId = new Randomium().nextString(15 );
            ...
            if( this.mysql().insertWithArray( this.alchemist().user().fragment().tabGlossaryNS(), glossaryData.getMap() ) > 0 ){
                return szClassId;
            }
        }
    ...
    public boolean deleteOneGlossary( String szClassId ) throws SQLException {
        ...
        return this.mysql().deleteWithSQL( this.alchemist().user().fragment().tabGlossaryNS(), szCondition ) > 0; // Using trigger
    }
    ...
    public void editGlossary() throws SQLException,IOException {
        ...
        this.checkResult(
                this.mysql().updateWithArray( this.alchemist().user().fragment().tabGlossaryNS(), glossaryInfo.getMap(), condition ) > 0
        );
    }
    ...
    public boolean abortOneCollection( String szClassId ) throws SQLException {
        ...
        return this.mysql().deleteWithSQL( this.alchemist().user().fragment().tabCollectionNS(), szCondition ) > 0;
    }
    ...
    public void appendNewRoot() throws IOException,SQLException{
        ...
        for( String word : words ){
            ...
            bRes &= this.mysql().insertWithArray( this.alchemist().user().fragment().tabRootsNS(), wordData.getMap() ) > 0;
        }
    ...
    public boolean deleteOneRoot( String szID ) throws IOException, SQLException {
        return this.mysql().deleteWithSQL( this.alchemist().user().fragment().tabRootsNS(), String.format(
    ...
    public void editWordProfile() throws SQLException,IOException {
        ...
        condition.put( "username", this.mszCurrentUser );
        this.checkResult(
                this.mysql().updateWithArray( this.alchemist().user().fragment().tabRootsNS(), data.getMap(),condition ) > 0
        );
    }
        ...
        if( jGlossaryWordList != null ){
            ...
            for( int i = 0; i < jGlossaryWordList.length(); i++ ){
                ...
                if( !indexSet.contains( szWord ) ){
                    ...
                    bRes &= this.mysql().updateWithArray( this.alchemist().user().fragment().tabRootsNS(), data, condition ) > 0;
                }
            }
        }
        else {
