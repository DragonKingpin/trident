...
public class SystemSpawner implements Pinenut {
    public static ArchServiceSystem spawnSystem ( String szMatrixPrototypeName, SystemServlet servlet ){
        ArchServiceSystem hMatrix = null;
        try {
            Class<?> pVoid = Class.forName( szMatrixPrototypeName );
            try{
                Constructor<?> constructor = pVoid.getConstructor( SystemServlet.class );
                hMatrix = (ArchServiceSystem) constructor.newInstance( servlet );
            }
    ...
    public static ArchConnectDispatcher spawnDispatcher (String szDispatcherPrototypeName, ArchServiceSystem system, RouterType routerType ){
        ArchConnectDispatcher archControlDispatcher = null;
        try {
            Class<?> pVoid = Class.forName( szDispatcherPrototypeName );
            try{
                Constructor<?> constructor = pVoid.getConstructor( ArchServiceSystem.class, RouterType.class );
